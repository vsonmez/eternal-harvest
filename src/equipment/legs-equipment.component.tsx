...
const LegsEquipment = () => {
  const { playerLegsItem, setPlayerLegsItem } = usePlayerEquipmentStore();
  const { setItemUnequipped } = usePlayerBagStore();

  const unEquip = () => {
    playerLegsItem && setItemUnequipped(playerLegsItem);
    setPlayerLegsItem(null);
  };
      ...
      {playerLegsItem && (
        <div className="flex flex-col gap-1 border">
          <div className="flex flex-col justify-between items-center gap-2 relative">
            <ItemNameImageComponent item={playerLegsItem} isCol />
...
export default React.memo(LegsEquipment);
