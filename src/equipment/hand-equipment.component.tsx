...
const HandEquipment = () => {
  ...
  const { playerHandItem, setPlayerHandItem } = usePlayerEquipmentStore();
  const { setItemUnequipped } = usePlayerBagStore();
  const unEquip = () => {
    playerHandItem && setItemUnequipped(playerHandItem);
      ...
      {playerHandItem && (
        <div className="flex flex-col gap-1 border">
          <div className="flex flex-col justify-between items-center gap-2 relative">
            <ItemNameImageComponent item={playerHandItem} isCol />
...
export default React.memo(HandEquipment);
